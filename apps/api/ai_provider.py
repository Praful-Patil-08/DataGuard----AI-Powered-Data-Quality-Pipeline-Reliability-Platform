...
class AIProvider(ABC):
    @abstractmethod
    def analyze(self, dataset_name: str, issues: List[Dict[str, Any]], affected_assets: List[str]) -> Dict[str, Any]:
        pass
...
class OpenAIProvider(AIProvider):
    ...
    def analyze(self, dataset_name: str, issues: List[Dict[str, Any]], affected_assets: List[str]) -> Dict[str, Any]:
        if not self.api_key:
            return MockProvider().analyze(dataset_name, issues, affected_assets)
        try:
...
Rules: explain what happened, root cause hypothesis, technical and business impact separately, concrete remediation, never claim you changed data."""
            ...
            fallback = MockProvider().analyze(dataset_name, issues, affected_assets)
