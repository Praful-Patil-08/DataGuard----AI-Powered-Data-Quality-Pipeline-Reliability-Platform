...
2. **Backend (`apps/api`)**:
   ...
   - `ai.py` + `ai_context.py` + `ai_provider.py` — **Phase 12** (AI Analyst hardening, human-in-the-loop):
     - Pipeline: `RAW DATA → DETERMINISTIC ENGINE → FACTS → AI CONTEXT BUILDER → LLM → STRUCTURED ANALYSIS → HUMAN APPROVAL`, no raw data to LLM, only aggregated facts.
     - `ai_context.py`: `build_ai_context` (scan, dataset, issues capped 20, schema_diff, drift_stats, quality_score, historical 3, downstream_assets/lineage_details 10, impact, dataset_metadata with fingerprint/columns, constraints `no_raw_data/must_ground/must_not_modify/must_require_approval`), sanitization `_sanitize` (prompt injection markers `SYSTEM:`, `IGNORE PREVIOUS`, ``` stripped), `validate_ai_output` (required fields, severity mapping, confidence clamp 0-1, affected_assets subset of downstream, `requires_human_approval` true for CRITICAL/WARNING, sanitize text, ensure root_cause not empty).
     - `ai.py`: `_sanitize_prompt`, `generate_fallback_from_context` (context-aware, grounded), `run_ai_analyst_with_context` (uses `build_ai_context` + `validate_ai_output`, fallback to `generate_fallback_from_context`), hardened `run_ai_analyst` (legacy, sanitized).
     - `ai_provider.py`: `analyze_with_context` for Mock/OpenAI/Gemini (prompt built from `Context` facts only, grounded instruction `Never claim you modified data`, `validate_ai_output` after), fallback to deterministic.
     - API: `POST /api/scans/{scan_id}/analyze` now builds full context (was legacy `dataset_name/issues/affected_assets/historical`, now `build_ai_context` with schema_diff/drift/quality/lineage/impact) and uses hardened `run_ai_analyst_with_context`.
     - Study: Strict engine-AI separation, human-in-the-loop, prompt injection guard, structured `AIAnalysisOutput` with `requires_human_approval`.
   ...
   - `ai.py` + `ai_provider.py`: OpenAI/Gemini/Mock analyst with structured `AIAnalysisOutput` (legacy).
...
6. **Integration & Demo Assets**:
    ...
    - `GET /api/datasets/{id}/history` now includes `quality_score`/`quality_dimensions` + `business_impact`, `GET /dashboard/*` 3 endpoints, `GET /scans/{id}/gate`, `GET /scans/{id}/score` (dimensions + summary), `GET /api/datasets/{id}/schema/history` + `.../schema/compare` (now with statistical drifts), `GET /api/baselines` + `.../baselines/{id}/compare/{dataset_id}` for explicit baselines, `GET /api/incidents` + `.../scans/{id}/incidents` for correlated incidents, `GET /api/lineage/graph` + `.../{dataset}/downstream/upstream` + `.../edges` for DB graph, `GET /api/impact/{dataset}/{column}` + `.../scans/{scan_id}/impact` for downstream business impact, `GET /api/reliability/overview` + `.../trends` + `.../datasets` + `.../columns` + `.../incidents` + `.../degradation` for historical reliability, `GET /api/anomalies` + `.../datasets/{id}/anomalies` + `.../scans/{id}/anomalies` for historical anomaly detection, `POST /api/scans/{scan_id}/analyze` **hardened** via `ai_context.build_ai_context` (facts only, no raw data, validated, prompt sanitized, grounded), `POST /scans/{id}/analyze` history-aware.
    - Contracts: `GET /api/contracts` + `POST /api/datasets/{id}/contracts/evaluate` for Soda-style declarative checks.
    - Tests: 117 hermetic `pytest` (7 core + 10 Watchtower + 13 contracts + 10 quality_score + 5 schema_evolution + 14 statistical_drift + 7 baselines + 8 incidents + 9 lineage_graph + 9 impact + 7 reliability + 9 anomaly + 9 ai_hardening) + `next build` 6 routes.
